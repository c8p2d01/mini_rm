...
/*   Updated: 2025/10/06 18:40:18 by cdahlhof         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	march(t_mrt *mrt, t_vec3d *ray, t_vec3d point, t_obj **hit, int *depth)
{
	//double pd = (min_dst(mrt->obj, point, hit));
	//if (pd >= RENDER_DISTANCE || (pd >= 0 && pd < 0.01) || *depth > 100)
	//	return (pd);
	//t_vec3d new_point = v_sum(point, v_product(v_unit(*ray), pd));
	//// printf("m, single %lf\n", pd);
	//*depth += 1;
...
int		color(t_mrt *mrt, t_obj *hit, t_vec3d *ray, double dst, bool print)
{
	//t_vec3d	b = init_vec3d(hit->r, hit->g, hit->b);
	//t_vec3d	to_l;
	//t_obj	*cast = &mrt->tmp;
	//int		cdepth = 0;
	//t_vec3d	norm;
	...
	//	default: norm = init_vec3d(1, 0 ,0);break;
	//}
	//double	ld;
	//float	f = 0;
	...
	//	if (cast != &mrt->tmp)
	//	{
	//		b = v_sum(b, v_product(v_product(init_vec3d(mrt->l[i]->r, mrt->l[i]->g, mrt->l[i]->b), mrt->l[i]->lr), diffuse));
	//		f++;
	//		if (print)
...
{
	//t_vec3d	ray = v_unit(single_ray(x, y, mrt->cam, scr));
